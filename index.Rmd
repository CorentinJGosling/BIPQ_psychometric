...
Je teste la normalité de la distribution des erreurs des items de la BIPQ. Si W \> 0.98, la distribution est proche d'une loi normale. -\> est ce que c'est nécessaire de toutes les tester et surtout est ce que c'est la bonne manière de regarder la distribution des erreurs ? =\> non on pourrait regarder la distribution des résidus des associations mais ça me semble OK de pas être plus royaliste que le roi (ok je viens de voir que c'est en dessosu, aprfait!)
...
plot_resid <- function(yvar, label, xvar="Score_BIPQ", min_n=10) {
  ...
  qqnorm(res, main = paste0("QQ ", label))
  qqline(res)
}

plot_resid("score_GAD7",       "GAD7 ~ BIPQ")
plot_resid("score_PHQ9",       "PHQ9 ~ BIPQ")
plot_resid("Score_whodas",     "WHODAS ~ BIPQ")
plot_resid("Score_autostigma", "SSS ~ BIPQ")
...
# Convertit l'objet summarytools en data.frame exploitable
tab <- as.data.frame(d)

# Les colonnes s'appellent généralement "Skewness" / "Kurtosis" (casse variable selon versions)
...
# -----------------------------
# 2) Data-visualisation globale avec dfSummary (mini histogrammes + NA, etc.)
# -----------------------------
dfs <- dfSummary(
  data_efa[vars],
  style      = "grid",
  graph.col  = TRUE,   # mini-graphiques dans la colonne Graph
  valid.col  = TRUE,
  na.col     = TRUE
)


# ou rendu console / HTML
print(dfs, method = "render")


# affichage skewness and kurtosis
skew_kurt_table%>%
  kbl() %>%
  kable_styling()%>%
...
Explications : alpha_result$total$raw_alpha : alpha global

r.drop : corrélation item–total corrigée (souvent on aime voir \> 0.30)

alpha_if_deleted : si supprimer l’item augmente beaucoup alpha → item possiblement problématique

Erreur affichée : Some items ( stigma_percept_4 stigma_percept_7 ) were negatively correlated with the first principal component and probably should be reversed.

Mais dans mon fichier les items sont déjà renversé dans le sens voulu
...
# items = data.frame d'items numériques
...
report_tbl %>%
  kbl(caption = "Descriptifs + Floor/Ceiling effects par item") %>%
...
Avec le professeur Morvan, nous avions déjà eu des pb pour faire 2 fateurs sur la BIPQ sur notre jeu de données. Ceà marchit uniquement lorsque nous fixions la covariance pour un item.
...
# IC de la correlation entre variables latentes
latcorPHQ <- parameterEstimates(fit_PHQ, standardized = TRUE, ci = TRUE)

# see result of interest
subset(latcorPHQ, lhs == "BIPQ" & rhs == "PHQ9" & op == "~~")[, 
...
# corrélation contrainte à 1
BIPQ ~~ 1*PHQ9
'

fit_PHQ_corr <- cfa(cfa_PHQ_corr,
                    data = data_phq,
                    estimator = "MLR", 
                    std.lv = TRUE,
                    missing   = "fiml")
...
Si le test est significatif :la contrainte (corrélation = 1) dégrade le modèle donc les facteurs ne sont pas équivalents
...
cfa_model_GAD <- '
BIPQ =~ stigma_percept_1 + stigma_percept_5 + stigma_percept_6 + stigma_percept_8 + stigma_percept_3 + stigma_percept_4 + stigma_percept_7 + stigma_percept_2
GAD7 =~ nerveux + inquiet + inquiet_exces + difficult_detente + agitation + tendance_contrarie + sentiment_peur
BIPQ ~~ GAD7'

fit_GAD <- cfa(cfa_model_GAD, 
               data = data_gad, 
               estimator = "MLR",                 
               std.lv = TRUE)

#summary(fit_GAD, fit.measures = TRUE, standardized = TRUE, rsquare = TRUE)
fitMeasures(fit_GAD, c("cfi", "tli", "rmsea", "srmr"))
...
# IC de la correlation entre variables latentes
latcorGAD <- parameterEstimates(fit_GAD_Missing, standardized = TRUE, ci = TRUE)

# see result of interest
subset(latcorGAD, lhs == "BIPQ" & rhs == "GAD7" & op == "~~")[, 
       c("est", "ci.lower", "ci.upper", "pvalue")]

```

Est ce mieux d'utiliser le modèle avec ou sans la gestion des NAs

#### with correlation = 1

est ce qu'on garde missing = "fiml" pour la GAD car elle a bcp de valeurs manquante
...
GAD7 =~ nerveux + inquiet + inquiet_exces + difficult_detente + 
...
fitMeasures(fit_GAD_1, c("cfi", "tli", "rmsea", "srmr"))
fitMeasures(fit_GAD_Missing_1, c("cfi", "tli", "rmsea", "srmr"))

lavTestLRT(fit_GAD_Missing, fit_GAD_Missing_1)
```
...
# IC de la correlation entre variables latentes
latcorSSS <- parameterEstimates(fit_SSS, standardized = TRUE, ci = TRUE)

# see result of interest
subset(latcorSSS, lhs == "BIPQ" & rhs == "SSS" & op == "~~")[, 
       c("est", "ci.lower", "ci.upper", "pvalue")]
```
