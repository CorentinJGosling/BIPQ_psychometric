...
table1(~ factor(Sexe) + factor(age_classe) + age + factor(situation_emploi_ini) + factor(situation_financiere) + factor(diplome_ini)+ factor(DEM_3_Aide) + Score_lubben, data=dat, caption=caption)
...
Quelques graphiques sur la distribution des erreurs : 


```{r, echo=FALSE, message=FALSE}

# test la normalité des erreurs (items BIPQ)
# Variables
x <- dat$stigma_percept_1
y <- dat$stigma_percept_2
...
par(mfrow = c(1,2))
hist(res, breaks = 20, main = "Histogram of residuals item2~item1", xlab = "Residuals")
qqnorm(res, main = "QQ-plot of residuals item2~item1")
qqline(res)
par(mfrow = c(1,1))
...
# Variables
x <- dat$stigma_percept_4
y <- dat$stigma_percept_5
...
# Visualisation de la distribution des erreurs
par(mfrow = c(1,2))
hist(res, breaks = 20, main = "Histogram of residuals item5~item4", xlab = "Residuals")
qqnorm(res, main = "QQ-plot of residuals item5~item4")
qqline(res)
par(mfrow = c(1,1))
...
# Visualisation de la distribution des erreurs
par(mfrow = c(1,2))
hist(res, breaks = 20, main = "Histogram of residuals item2~item7", xlab = "Residuals")
qqnorm(res, main = "QQ-plot of residuals item2~item7")
qqline(res)
par(mfrow = c(1,1))
...
Au vu des résultats, est ce qu'il est pertinent d'utiliser pearson ou non ?? Certaines distributions se rapprochent d'une normale tandis que d'autre pas du tout (exemple avec item 2 et 7)

J'ai vérifié les erreurs d'autres construits, en standardisant les scores au préalable (au score global pour la convergent / divergent validity ) 
